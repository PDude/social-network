  ...
  return (
    <NavLink to={`/profile/${id}`} className={style.user_item}>
      ...
      <div className={style.user_info}>
        ...
        <p className={style.status}>{status != null ? status : statusPlug}</p>
        <p className={style.location}>
          {/* <span className={style.country}>{props.country}</span>, <span className={style.city}>{props.city}</span> */}
          <span className={style.country}>Skyrim</span>,{' '}
          <span className={style.city}>Solitude</span>
        </p>
      </div>
      {props.followed ? (
        <button
          ...
          className={`${style.subscriptionBtn} ${style.followed}`}>
          Unfollow ✔
        </button>
      ) : (
        <button
          ...
          className={style.subscriptionBtn}>
          Follow
        </button>
      )}
      {followingInProgress.some((userId) => userId === id) ? (
        <Preloader />
      ) : null}
    </NavLink>
  )
